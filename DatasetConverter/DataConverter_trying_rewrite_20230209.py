...
class DatasetGenerator():
    ...
    def __init__(self,
        ...
        self.esJob = esJob
        if SQLFile != "":
            self.StasticSwitch = False
        else:
            self.StasticSwitch = StasticSwitch
        ...
        #raise Exception
        self.SampleReaders = self.BuildSampleReaders()
    ...
    def BuildSamplesDfFromPaths(
        ...
        '''
        SampleReaders = self.BuildSampleReaders()
        print("SampleReaders",SampleReaders)
        ...
        MPresult = multicoreJob(
            SampleReaders, nProcess=nProcess).run()
        ...
        #儲存標籤映射函數。
        for y in sorted(set([self.LabelConvertDict[x] for x in LabelList])):
    ...
    def BuildFileList(self):
        ...
        print("="*50)
        print("In BuildFileList, self.ROOTPATHList",self.ROOTPATHList)
        if self.SQLFile == "":
    ...
    def BuildSampleReaders(self):
        DTBJobs = []
        ESidSet = set(self.ESidList)
        #print("In BSReaders, self.fileList",self.fileList)
        for (file,esJob) in [
    ...
    def SplitDataset:
        ...
        def run(self):
            ...
            for key in nDict.keys():
                ...
                if key == "test":
                    if len(FixfiL) > 0:
                        FT_df = DatasetGen.BuildSamplesDfFromPaths(
    ...
    def proc(self,):
        ...
        df = self.BuildSamplesDfFromPaths(
            #ROOTPATHList = self.ROOTPATHList,
            #SQLFile = self.SQLFile,
            #DataAugmentationGoal = self.DataAugmentationGoal,
            #OUTPUTMAIN = self.OUTPUTMAIN,
            #OUTPUTMAIN_Counter = self.OUTPUTMAIN_Counter,
            #nProcess = nProcess,
            #DCkwargs = DCkwargs
            )
...
if __name__ == '__main__':
    ...
    MES = "開始產製資料集檔案。"
    MPlogger.logW(MES)
    print(f"{MES}, ROOTPATHList",ROOTPATHList)

    DatasetGenerator(
        datasetSubDir = datasetSubDir,
        ROOTPATHList = ROOTPATHList,
        SQLFile = "",
        StasticSwitch = StasticSwitch, #資料集轉換時是否要做統計
        esJob = dict(), #es_token,indexname,startDay,endDay
        RemoveDumpArticle = True,
        ReadQuery = "",
        OUTPUTMAIN = os.path.join(
