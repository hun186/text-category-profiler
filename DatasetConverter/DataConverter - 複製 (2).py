...
def BuildSamplesDfFromPaths(
    ...
    df = DictRowsListToDF(
        rows_list,start_time=start_time,RemoveDumpBasedOnCols=['OutLabel','text'])
...
if __name__ == '__main__':
    ...
    if args.WeiTechworkID != "":
        ...
        try:
            ...
            #FNrePatListForPicker = ["^dataset_total_with_filename_FixedTest.sql3$"]

            #for file in FileNamePicker(dirList=[WTBertDatasetSubDir],FNrePatList=FNrePatListForPicker):
                
                #des = os.path.join(WTdatasetDBDir,getFNFromFullPath(file))
