...
def LoadEmbassyEmails():
    ...
    for term in df["Email"]:
        ...
        termSP = [x for x in termSP if any(
            ["mfa" in x,"gov" in x, "mofa" in x
             ])]
...
def GeneratePool(LocaleName):
    ...
    global PhoneNumberTitles,PhoneNumbers,EmailTitles
    global CompanyEmails,CompanyNames,HomepageUrls
    #global EmbassyEmails
    global AddressTitles,Addresses,Cities
    global UserAgents
    global RandomDatetimes
    #global MimeTypes
    global MessageIDs,ContentTransferEncodings, Charsets
    ...
    ContentTransferEncodings = ["8bit"]*10+["binary"]*3+["base64"]*3+["quoted-printable"]*3+["7bit"]+["x-token"]
...
def GenerateContactInformation(LocaleName="en_US"):
    ...
    SubDir = os.path.join("#T#[Uncertainty-Contact Information]",LocaleName)
    MKDIR(SubDir)
    for i in range(nGen):
        ...
        with open(OutputFN,'wt',encoding='utf-8') as f:
            f.write(CTX+"\n")
#%%寄件者、收件者、副本Email清單
def UserEmailSets(label="Email Header-Email Address"):
    FromList = []
    MailtoList = []
    CCtoList = []
...
#%%Email標頭電郵資訊生成函式
def GenerateEmailAddressInEmailHeader(nGen,LocaleName="en_US",label="Email Header-Email Address"):
    print(f"Generating Email Address In Email Header for {LocaleName}")
    GeneratePool(LocaleName)
    SubDir = os.path.join(f"#T#[{label}]",LocaleName)
    #if mode=="general":
        #SubDir = os.path.join("#T#[Email Header-Email Address]",LocaleName)
    #elif mode=="embassy":
        #SubDir = os.path.join("#T#[Email Address-Embassy]",LocaleName)
    MKDIR(SubDir)
    for i in range(nGen):
        OutputFN = os.path.join(SubDir,f"Email Address_{i}.txt")
        FromList,MailtoList,CCtoList = UserEmailSets(label=label)
...
def GenerateEmailHeader(nGen,LocaleName="en_US"):
    ...
    for i in range(nGen):
        ...
        CTXList = [
            ...
            {ReferencesStr}
            {InReplyToStr}
            Content-Type: {fake.mime_type()}; charset={random.choice(Charsets)}
        ...
        for (CTXsubDir,CTX) in CTXList:
            ...
            with open(OutputFN,'wt',encoding='utf-8') as f:
                f.write(CTX+"\n")
    #print("ContentTransferEncodings",random.choice(ContentTransferEncodings))
...
if __name__=='__main__':      
    ...
    #%%聯絡資訊
    for LocaleName in [
        'zh_TW','zh_CN','en_US','fr_FR','de_DE','es_ES','jp_JP','ru_RU','ko_KR',
        ]:
        #聯絡資訊
    ...
    for LocaleName in [
        ...
        for label in ["Email Header-Email Address","Email Address-Embassy"]:
            GenerateEmailAddressInEmailHeader(nGen,LocaleName=LocaleName,label=label)
