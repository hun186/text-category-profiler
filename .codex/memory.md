...
- 來源 adapter：`adapt_czj_sample_records()` 隔離 CZJ samples SQLite records 的 persisted `index` 移除，保留 row order、額外 provenance 與 worker 回傳契約，malformed row 在 downstream side effects 前帶 index 失敗。
