...
| Dataset converter | 建立分類器需要的 dataset／SQLite；CLI 由 `main(argv=None)` 與 runtime bootstrap 進入；filesystem source 以 immutable `SourceSpec` 描述 role 與 discovery policy；sliced-text reader 透過純 `sample_pipeline` 清整 segment layout、判定 malformed-segment label、選取 regex-based input label 並組裝 canonical row，reader job results 再由該 pipeline 彙整並交給 dependency-free `sample_schema` 驗證共同必要欄位，最後轉 DataFrame；provenance path policy 位於 dependency-free `source_metadata`；切分前依 label/text 去重，以可測試的 split plan 建立 train/dev/test，再僅對 train 的少類樣本擴增 | 原始文字、固定測試資料、ES job config | `train.tsv`、`dev.tsv`、`test.tsv`、dataset DB | `DatasetConverter/DataConverter.py`, `DatasetConverter/source_collection.py`, `DatasetConverter/source_metadata.py`, `DatasetConverter/sample_pipeline.py`, `DatasetConverter/sample_schema.py`, `DatasetConverter/dataset_split.py` |
