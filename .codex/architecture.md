...
| Dataset converter | 建立分類器需要的 dataset／SQLite；CLI 由 `main(argv=None)` 與 runtime bootstrap 進入；filesystem source 以 immutable `SourceSpec` 描述 role 與 discovery policy；切分前依 label/text 去重，以可測試的 split plan 建立 train/dev/test，再僅對 train 的少類樣本擴增 | 原始文字、固定測試資料、ES job config | `train.tsv`、`dev.tsv`、`test.tsv`、dataset DB | `DatasetConverter/DataConverter.py`, `DatasetConverter/source_collection.py`, `DatasetConverter/dataset_split.py` |
