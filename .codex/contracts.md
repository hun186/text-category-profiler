...
- 輸入：`--train/-tr`、`--test/-ts`、`--task`、`--WorkPoolROOT/-WPRoot`、`--BertDatasetSubDir/-BertDataDir`、`--TopicTreeDir/-TopicTreeDir`、`--TopicTreeFiles/-TopicTreeFiles`、`--modelDir/-mdlDir`、`--FixedTestPATH/-FTPath`、`--SaveOptimizer/-SaveOptimizer`、WeiTech work pool 相關參數、model type 與視覺化參數等。Production `ModelType` canonical default 為 `PytorchMMBERT`，仍可明確指定 `PytorchXLM`。`--doctor` 是 activation-free resource preflight；`--self-test {isolated,real}` 也在 activation 前分流並執行 disposable child root。Doctor 的 `--require-cuda` 要求 availability，real self-test 則要求 classifier 明確回報 `device=cuda:<n>`；diagnostic flags 不轉送給 child/stage CLI。`--SaveOptimizer` 預設為 `false`，因此 Hugging Face checkpoint 不保留 `optimizer.pt`；需要續訓狀態時可傳 `--SaveOptimizer true`。
