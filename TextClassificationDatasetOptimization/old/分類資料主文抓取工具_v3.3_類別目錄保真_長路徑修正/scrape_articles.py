...
def main() -> int:
    ...
    parser.add_argument(
        "--max-path-chars", type=int, default=240,
        help=(
            "完整輸出路徑安全上限；預設 240 以相容未啟用 Long Paths 的 Windows。"
            "只會縮短文章檔名，絕不改寫 #T#[正式類別 label]。設 0 可停用此限制。"
        ),
    )
    ...
    summary = [
        ...
        f"每類文章上限：{category_limit_text}",
        f"完整路徑安全上限：{'停用' if args.max_path_chars == 0 else str(args.max_path_chars) + ' 字元'}",
        f"原始記錄：{len(entries)}",
