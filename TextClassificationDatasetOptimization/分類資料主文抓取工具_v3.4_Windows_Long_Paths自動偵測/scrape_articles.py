...
def sanitize_filename_component(value: str) -> str:
    """Sanitize an ordinary filename component.

    This function is intentionally NOT used for category directory names.
    Category labels are formal identifiers and must never be silently rewritten.
    """
...
def truncate_with_hash(value: str, max_len: int) -> str:
    ...
    if prefix_len < 1:
        return digest[:max_len]
    return value[:prefix_len].rstrip() + suffix


def safe_component(value: str, max_len: int = 120) -> str:
    """Sanitize and, when needed, shorten ARTICLE filename components only."""
    return truncate_with_hash(sanitize_filename_component(value), max_len)


def exact_category_component(category: str) -> str:
    """Return the formal category label unchanged or fail loudly.

    The old v3.2 code called safe_component(category, 100), which silently turned
    long labels into values such as ``...-Boar__94b71f9c``.  That breaks the
    invariant that the #T#[label] directory must equal the formal label.  v3.3
    therefore never truncates, hashes, normalizes, or substitutes characters in
    category labels.
    """
    if not category:
        raise ValueError("類別 label 為空，無法建立 #T#[類別] 目錄。")
    if category != category.strip():
        raise ValueError(f"類別 label 前後含空白，程式不會自動改寫：{category!r}")
    if re.search(r'[<>:"/\\|?*\x00-\x1f\x7f]', category):
        raise ValueError(
            "類別 label 含 Windows 目錄不可用字元；為避免破壞正式 label，"
            f"程式拒絕自動替換：{category}"
        )
    if category.endswith((" ", ".")):
        raise ValueError(f"類別 label 不能以空白或句點結尾：{category}")
    if category.upper() in WINDOWS_RESERVED:
        raise ValueError(f"類別 label 是 Windows 保留名稱：{category}")
    directory = f"#T#[{category}]"
    if len(directory) > 255:
        raise ValueError(
            "#T#[類別] 單一目錄名稱超過 Windows 常見 255 字元元件上限；"
            "程式不會截斷正式 label。請先重新檢視類別命名。"
        )
    return category


def category_dir_name(category: str) -> str:
    return f"#T#[{exact_category_component(category)}]"


WINDOWS_LONG_PATHS_REG_PATH = r"SYSTEM\CurrentControlSet\Control\FileSystem"


def windows_long_paths_enabled(*, os_name: Optional[str] = None, winreg_module=None) -> bool:
    """Return whether Windows Long Paths is enabled in the system registry.

    Windows exposes the setting as the registry value LongPathsEnabled under
    HKLM\\SYSTEM\\CurrentControlSet\\Control\\FileSystem. Registry access failures are treated conservatively as
    disabled so the existing 240-character compatibility budget remains active.
    """
    ...
    try:
        ...
        access = getattr(winreg_module, "KEY_READ", 0)
        with winreg_module.OpenKey(
            ...
            value, _value_type = winreg_module.QueryValueEx(key, "LongPathsEnabled")
        return int(value) == 1
    except (ImportError, OSError, TypeError, ValueError):
        return False


def resolve_max_path_chars(
    requested: Optional[int],
    *,
    long_paths_enabled: bool,
    fallback: int = 240,
) -> int:
    """Resolve the effective path budget while honoring explicit CLI overrides."""
    if requested is not None:
        return requested
    return 0 if long_paths_enabled else fallback


def make_output_rel_path(
    output_dir: Path,
    category: str,
    title: str,
    ...
    *,
    ...
    """Build a path while preserving the category directory exactly.

    If a legacy Windows path budget is requested, only the article filename is
    shortened.  The formal category label is never changed.  Set
    ``max_path_chars=0`` to disable the legacy budget on systems with long-path
    support.
    """
    category_dir = category_dir_name(category)
    clean_title = sanitize_filename_component(title)
    key = clean_title.casefold()
    used[category][key] += 1
    number = used[category][key]
    duplicate_suffix = "" if number == 1 else f"__{number}"

    base_limit = title_max_len
    if max_path_chars > 0:
        abs_category_dir = output_dir / category_dir
        # Budget includes path separator and .txt extension.  The duplicate
        # suffix is reserved before shortening the title.
        fixed = len(str(abs_category_dir)) + 1 + len(duplicate_suffix) + len(".txt")
        available = max_path_chars - fixed
        if available < 8:
            raise ValueError(
                "即使把文章檔名縮到最短，完整輸出路徑仍超過目前的"
                f" {max_path_chars} 字元安全上限。類別目錄不會被改寫：{category_dir}。"
                "請把 --output-dir 指到更短的路徑（例如 C:\\AT），或在已啟用"
                " Windows Long Paths 的環境使用 --max-path-chars 0。"
            )
        base_limit = min(base_limit, available)

    base = truncate_with_hash(clean_title, max(8, base_limit))
    filename = f"{base}{duplicate_suffix}.txt"
    rel_path = Path(category_dir) / filename

    if max_path_chars > 0 and len(str(output_dir / rel_path)) > max_path_chars:
        raise AssertionError("輸出路徑預算計算錯誤")
    return rel_path
...
def main() -> int:
    ...
    parser.add_argument(
        "--max-path-chars", type=int, default=None,
        help=(
            "完整輸出路徑安全上限。未指定時會在 Windows 自動偵測 LongPathsEnabled："
            "已啟用則自動使用 0（停用舊式 240 字元預算），未啟用或無法確認則使用 240。"
            "只會縮短文章檔名，絕不改寫 #T#[正式類別 label]。明確指定本參數時以指定值為準。"
        ),
    )
    ...
    if args.max_path_chars is not None and args.max_path_chars < 0:
        parser.error("--max-path-chars 不可小於 0；0 表示停用舊式 Windows 路徑長度預算")

    long_paths_enabled = windows_long_paths_enabled()
    effective_max_path_chars = resolve_max_path_chars(
        args.max_path_chars, long_paths_enabled=long_paths_enabled
    )
    if args.max_path_chars is None:
        if os.name == "nt" and long_paths_enabled:
            path_mode_text = "Windows Long Paths 已啟用；自動停用 240 字元路徑預算"
        elif os.name == "nt":
            path_mode_text = "Windows Long Paths 未啟用或無法確認；自動使用 240 字元安全上限"
        else:
            path_mode_text = "非 Windows 環境；沿用 240 字元相容安全上限"
    else:
        path_mode_text = (
            "使用者指定 --max-path-chars 0；停用舊式路徑預算"
            if effective_max_path_chars == 0
            else f"使用者指定 --max-path-chars {effective_max_path_chars}"
        )
    ...
    if args.dry_run:
        preview_used_names: dict[str, Counter[str]] = defaultdict(Counter)
        ...
        for i, e in enumerate(entries, 1):
            if e.index_kind:
                ...
                output_file = str(make_output_rel_path(
                    output_dir, e.category, e.title, preview_used_names,
                    max_path_chars=effective_max_path_chars,
                ))
            preview_rows.append({
                "序號": i,
                "狀態": status,
    ...
    for i, task in enumerate(retained_tasks, 1):
        ...
        task_rel_paths[task] = make_output_rel_path(
            output_dir, task.category, task.title, used_names,
            max_path_chars=effective_max_path_chars,
        )
    ...
    summary = [
        ...
        f"每類文章上限：{category_limit_text}",
        f"完整路徑安全上限：{'停用' if effective_max_path_chars == 0 else str(effective_max_path_chars) + ' 字元'}",
        f"路徑模式：{path_mode_text}",
        f"原始記錄：{len(entries)}",
    ...
    summary += [
        ...
        "- #T#[正式類別 label] 目錄名稱永不截斷、雜湊或自動替換；路徑過長時只縮短文章檔名。",
        "- 若類別 label 本身含 Windows 不合法字元或單一目錄元件過長，程式會明確報錯而不是偷改類別名。",
