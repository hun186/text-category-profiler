...
class VisDatatableRowsListBuilder:
    ...
    def run(self):
        ...
        for file in self.SrcList:
            ...
            CMP = GetClassOfMostPieces(segTuples,self.InfoScoreTable)
            rowDict["Class Of Most Pieces"] = \
                f"{self.LabelSep}{CMP}{self.LabelSep}"
            CHS = GetClassOfHighestScore(segTuples,self.InfoScoreTable)
            rowDict["Class Of Highest Score"] = \
                f"{self.LabelSep}{CHS}{self.LabelSep}"
            rowDict["Number of Exempt Pieces"] = nExemptPieces
...
class VisDatatableDFTransformer:
    ...
    def run(self):
        ...
        bar_df["Number of Exempt Pieces"].replace({0: ""}, inplace=True)
        #依分數區間篩濾
        ...
        #bar_df_saveTemp = bar_df_saveTemp[PreambleCols]
...
def Build_VisDatatable_style(df, ColorDict, BinMissionDict):
    ...
    style_cell_conditional = [
        ...
        ]+[
        {
            ...
            'if': {'column_id': "Number of Exempt Pieces"},
            'width': '12px',
            'minWidth': '12px',
        },{
            'if': {'column_id': "Date"},
...
def Build_VisDatatable(
    ...
    for file in PartDF['File']:
        ...
        try:
            ...
            MES = f"When Apply the query {query} to build segTuples with {sql3File}, "
            MES += f"the following error occurs: \n {e}"
            MPlogger.logW(MES)
        ...
        query = f'SELECT MAX(PartNO) FROM sampleSrc \
            WHERE File = "{file}";'
        #print("list(sqlite3Query(sql3File,  query = query))",list(sqlite3Query(sql3File,  query = query)))
        try:
            MaxPN = min(
                sqlite3Query(sql3File,  query = query,ListForm=True)[0]+1,300)
        except Exception as e:
            MES = f"When Apply the query {query} to get MaxPN, "
            MES += f"the following error occurs: \n {e}"
            MPlogger.logW(MES)
            MaxPN = 1
        ...
        infer_pieces = [None]*int(min(CutRange[1]+1,CutRangeMax))
...
def BuildPredsdf(sql3File, FileList):
    ...
    '''
    FileList = [x if x.endswith(".txt") else x+".txt" for x in FileList]
    colList=','.join(cols)
...
class VisDatatableDFBuilder:
    ...
    def run(self,SrcList):
        ...
        LenHighScoreFileUPD = min(
            int(8*40000/len(FileList))*125,
            len(FileList))#前面為12核24緒估1分鐘的量
        ...
        #SimilarityList = multicoreJob(
            #DTBJobs, nProcess=3).run()
        #print("SimilarityList b4",SimilarityList)
        ...
        #print("SimilarityList af",SimilarityList)
        #raise Exception
        #similarFilelist = zip(*MPresult)
        #print("SimilarityList[:10]",SimilarityList[:10])
        ...
        #print("len(CMP)",len(CMP))
        #raise Exception
        
        
        #print(CMP)
        #print("CMP b4",CMP.keys())
        #print("NIC b4",NIC)        
        ...
        #rowslist = rowslist[0]
        #rowslist, InfoScoreTableList = MPresult[0],MPresult[1]
        ...
        df = VisDatatableDFTransformer(
            ...
            InfoScoreSumLowerBound=self.InfoScoreSumLowerBound,
            InfoScoreSumUpperBound=self.InfoScoreSumUpperBound,
            FixedTestFileBound=self.FixedTestFileBound).run()
        
        #print("df",df)
        #print("Start to ComputeTwins.")
        #ShowElapsedTime(start_time)
        #df = ComputeTwins(df,self.sql3File,self.sqlCols)
        #print("Finished ComputeTwins.")
        #ShowElapsedTime(start_time)
        
        #dfOutputer(df[PreambleCols],self.SessionPreambleColsOPTMain).run()
        #print("OUTPUTMAIN",self.SessionPreambleColsOPTMain)
        return df
...
def table_update(
    ...
    #ShowingFileScoreDF_json = ShowingFileScoreDF.to_json(date_format='iso', orient='split')
    return [VisData_children]+[CutRangeMax]+[marks]+[ShowingFilePVTCard_children]
#============================================================   
...
def update_output(
    ...
    #自傳文本推論
        ...
        try:
            ...
            raise PreventUpdate            
        src = os.path.join(SessionDatasetDir, FN)
        desDir = os.path.join(SessionDatasetDir, "FT", "Using", "#T#[Scrap]")
        MKDIR(desDir)
        des = os.path.join(desDir, FN)
        print("src,des", src,des)
        shutil.move(src,des)
        ExtractZip(des)
        
        
        FTDir = os.path.join("..","BertScript",SessionDatasetDir, "FT")
        BertDataDir = os.path.join("..","BertScript",SessionDatasetDir,"dataset")
        os.chdir("../DatasetConverter")
        os.system(
            f"python DataConverter.py -ts \
        ...
        '''
        #print("InfoScoreSumLowerBound:",InfoScoreSumLowerBound)
        #print("InfoScoreSumUpperBound:",InfoScoreSumUpperBound)
        VDT_DFBuilder = VisDatatableDFBuilder(
            nProcess=nProcess,
            tpcTree=tpcTree,
            BinMissionDict=BinMissionDict,
        ...
        df = VDT_DFBuilder.run(SrcList)
        df['Date'] = df['Date'].apply(lambda x:"🌎"+x if len(x)>0 else x)        
        '''


        #ColortableArray = ColorDF.to_dict('records')
        #style_cell_conditional=Colortable_style_cell_conditional,
    ...
    if button_id in ["btn_InfoScore","Uploaded Filename"]:
        cms = cmapSet()
        ColorDict = {}
        LabelList = []
...
#=========客戶端自傳推論相關callback==================================
datasetDir_VisSelf = "dataset_VisSelfService"            
#UPLOAD_FOLDER_ROOT = os.path.join(datasetDir_VisSelf,"Uploads")
UPLOAD_FOLDER_ROOT = datasetDir_VisSelf
...
#def serve_layout(datasetDir):
    ...
    #print("TRsql3FileDatasetDir",TRsql3FileDatasetDir)
    #raise Exception
        ...
        html.Div([
            ...
            #html.Div([
                #dbc.Card(
                    #[
                    #dbc.CardBody(
                        #children=html.Button("依分數更新篩選源", id="btn_InfoScore")
                        #),
                    #html.Hr(),
                    #],
                    #style={"width": "2"},
                    #)],
                #className = 'col-2',style = {'verticalAlign': 'center'})
                   #,width = 1),
        ],
        className = 'row'
        ),
...
if __name__=='__main__':
    ...
    datasetDir, outputDir = datasetDirOutputDirPickers.proc(modelType = modelType)
    ...
    '''
    nLeftFileChunk = 0
    #建立文本推論結果可視化表格，並於BuildRowsList函式中更新類別分數表:全域變數InfoScoreTable。
    #豁免機制，過程中可能會新增標籖，故回傳新的InfoScoreTable。
    #rowslist[0]:
    #一列的值
    ...
    '''
    rowslist,InfoScoreTable = VisDatatableRowsListBuilder(
        sql3File, SrcList, sqlCols=sqlCols,InfoScoreTable=InfoScoreTable,
        nLeftFileChunk=nLeftFileChunk).run()
    '''
    ...
    SelectedFNPatList = []
    for file in OSWALK(FixedTestDir):
        if getFNFromFullPath(file) == "select.txt":
            if os.path.isfile(file):
                PatList = open(file,'rt',encoding='utf-8').readlines()
                SelectedFNPatList.extend(PatList)
    ...
    '''
        ...
        LabelList.extend(ExemptLabelList)
        rowslist,ExemptLabelList = BuildRowsList(sql3File, SrcList, cols=cols)
    '''
    LabelList = sorted(set(LabelList))
    LabelList.extend(ExemptLabelList)
