...
def dfToSQL(SQLname, df, SavingDfColumns = [], IndexCols=[]):
    ...
    table = 'sampleSrc'
    #if_exists 選擇 replace，若是sampleSrc 這個 table 已存在資料庫
    #將sampleSrc 表刪除並重新創建 寫入 sql_df 的資料
    sql_df.to_sql(name=table, con=cnx, if_exists='replace')
    #創造index，以提升讀取速度。
    for col in IndexCols:
        createIndex = 'CREATE INDEX "{}_Index" ON "sampleSrc"("{}");'.format(col,col)
...
class dfOutputer:
    def __init__(self, df, OMFN, IndexCols=[]):
        self.df = df
        OMFN = os.path.join('/tmp',OMFN)
        self.OMFN = OMFN
...
def dfFromSQLite3(sql3File,
    ...
    #print(df.head())
    conn.close()
    print("Finished Loading the table {}".format(name))
    return df
