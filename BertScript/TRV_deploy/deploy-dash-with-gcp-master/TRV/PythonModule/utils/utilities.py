...
def getPathFromFN(FN):
    return PathSEP(FN).join(pathSpliter.proc(FN)[:-1])
...
def getFNFromFullPath(FN):
    ...
    '''
    #result = FN.split(PathSEP(FN))[-1]
    result = pathSpliter.proc(FN)[-1]
    if result == "":
        result = "FN_Detection_Failure"
    return result
...
def ExtractZip(InputFN):
    files = zf.ZipFile(InputFN, 'r')
...
'''
class ActorUI(QWidget):
    def __init__(self, parent= None, nProcess=1, headless=False):
        self.nProcess = nProcess
        self.headless = headless
        super(QWidget, self).__init__(parent)
        self.SingleCellHeght = 220
        self.SingleCellWidth = 420
        self.initUI()
        threads = []
    def countScreenSize(self):
...
if __name__ == '__main__':
   FN = 'd:/C_GenesisSearch\\Korean\\Page 1\\(1) 김홍종 - 미적분학1. 1-서울대학교출판문화원 (2020).pdf'
   print(ShortenFN(FN))