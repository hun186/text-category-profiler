...
InfoScoreSumLowerBound=-999999999999999999
FixedTestFileBound=30000000
...
def sql3LoadForFileList(sql3File, SrcList, 
    ...
    for file in SrcList:
        ...
        ClassOfMostPieces = GetClassOfMostPieces(segTuples)
        ClassOfHighestScore = GetClassOfHighestScore(segTuples)
        #print("file",file)
        #raise Exception
        #InfoScoreSum = GetInfoScoreSum(segTuples)
        #InfoScoreMean = int(InfoScoreSum/len(segTuples))
        InfoScoreSum,InfoScoreMean=GetInfoScoreStastic(segTuples)
        #在第一欄補上Rating預設值""
        #在第三欄補上nMatchingSpecBlock預設值0
        result.append(["",
                       InfoScoreSum,
                       InfoScoreMean,
                        0,
...
def build_ChunkDF(sql3File,
    ...
    cols=['PartNO','pred_Type','text']
    rowslist = sql3LoadForFileList(sql3File, SrcList, cols=cols)
...
def Build_MissionTable(MissionDataArray):
    ...
    return dash_table.DataTable(
        ...
        columns=[{'id': str(c), 'name': str(c)} for c in ['Mission', 'Expiry Date', 'Topics']],
...
def Build_Pred_Block(df, ColorDict):
    print("="*50)
    ShowElapsedTime(start_time)
...
def RowsFilter(df, selectedLabels, PiecesBound = [1, 100]):
    #ShowingRows = []
    #print("IN RF, selectedLabels",selectedLabels)
    print("IN RF, PiecesBound",PiecesBound)
    def CountMatchingBlock(df, selectedLabels):
    ...
    if selectedLabels == []:
        ...
        nMatchingBlockSeries = CountMatchingBlock(df, selectedLabels)
...
PreambleCols = ["Rating",
                ...
                "Class Of Most Pieces","Class Of Highest Score","File"]
...
try:
    df = build_ChunkDF(sql3File,InfoScoreSumLowerBound=InfoScoreSumLowerBound)
except Exception as e:
...
def LoadMissionData(FN='SPCMission.txt',Cols=['Mission','Expiry Date','Topics']):
    result = []
    with open(FN,'rt',encoding='utf-8') as f:
        for line in f:
            entries = line.rstrip().split(',')
            result.append({'Mission':entries[0],
                           'Expiry Date':entries[1],
                           'Topics':str(entries[2:])})
    return result
...
# =============================================================================
            ...
            dbc.Card(
                [
                    ...
                    dbc.CardBody(
                        ...
                        children = Build_Pred_Block(FilteredDF, ColorDict),
                        #children = [dcc.Graph(
                            #id="Sunburst-graph",
    ...
    dbc.Card(
        ...
        dbc.CardHeader(children = 
            ...
            #rc.Col(html.H2("Samples Predictions:"),width=3),
            rc.Col(rc.Row([html.H2("total Number:"),
                           html.H2(children=f'{nSamples}')
                           ]),width=3),
            #rc.Col(rc.Row([html.H2("Select Topics:"),
                           #html.H2(id = "selectedLabels",children=f'{selectedLabels}')
                           #]),width=3),
...
def FilteredDF_update(
    ...
    return [FilteredDF_json]+[FilteredDF.shape[0]]+[
...
def table_update(
    ...
    FilteredDF = pd.read_json(FilteredDF_json, orient='split')
    PredSum_children = Build_Pred_Block(FilteredDF, ColorDict)
    ShowElapsedTime(start_time)
