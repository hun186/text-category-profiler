...
#from utils.df_utils import WeiTechFormatOutputer
...
from utils.utilities import reCombiner
...
from utils.DataConverter_utils import GetRoots
...
#print("finished lading TwinsClassifier")
...
def PerformanceColExt(df):
    ...
    df["Recommanded"] = df.apply(
        lambda x: "推" if x.InfoScoreSum>=1000 else "未推", axis=1)
    ...
    def run(self):
        ...
        for file in self.SrcList:
            ...
            try:
                ...
                continue
            #print("FileSrcQueryList",FileSrcQueryList)
            ...
            #print("SelectedFNPatList",self.SelectedFNPatList)
            #print("rowDict[File]",rowDict["File"])
            #time.sleep(10)
...
#def Build_LabelSelector(tpcTree,Parents=None):
#def Build_LabelSelector(app,df):
def Build_LabelSelector(app):
    #return layout_interactivity.create_layout(app,df)
    return layout_interactivity.create_layout(app)

    if Parents == None:
        Parents = GetRoots(tpcTree, OnlyLettersDigitsLabels = False)
    for tpc in Parents:
        subtpcs = sorted(GetSubTopics([tpc], tpcTree, depth=1))    
        print(subtpcs)
        print("="*50)
...
class VisDatatableDFBuilder:
    ...
    def run(self,SrcList):
        ...
        LenHighScoreFileUPD = min(baseNum,len(FileList),TwinsHighScoreNoUBD)
...
def update_output(
    ...
    #自傳文本推論
        ...
        os.system(
            #f"python DatasetConverter/DataConverter.py -ts \
            f"python TCFMain.py -ts \
...
#print("datasetDir_VisSelf",datasetDir_VisSelf)
...
if __name__=='__main__':
    ...
    for mission in MissionDataArray:
        try:
            ...
            MES = f"When loading mission {mission}, the following error occurs:\n{e}"
            MPlogger.logW(MES=MES,logFile="Test_result_Vis.log")
